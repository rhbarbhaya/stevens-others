...
avg_total =  np.array([np.mean(Total[0:i]) for i in range(NUM_RUNS)])
confidence_int = 1.96*np.array([stats.sem(Total[0:i]) for i in range(NUM_RUNS)])
end_level = np.array([np.mean(remaining[0:i]) for i in range(NUM_RUNS)])
mean_estimate = avg_total
lower_limit = avg_total - confidence_int
upper_limit = avg_total + confidence_int

print "Mean Balance = {:.3f} with Confidence Interval of +/- {:.3f}".format(avg_total[-1], confidence_int[-1])
print "Average of inventory at the end of the simulation = {:.2f}".format(end_level[-1])
...
#import csv
#
#with open('inventory.csv', 'wb') as output:
